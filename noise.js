...
function noise(x,y){
  ...
  let cornerOff = [{x: 0,y: 0},{x: 1,y: 0},{x: 1,y: 1},{x: 0,y: 1},]
  let dotProducts = new Array(4)
  for(let i = 0; i < cornerOff.length; i++){
    let gradientVector = gradientArray[cornerOff[i].y + tile.y][cornerOff[i].x + tile.x]
    let offset = {x: (x - (tile.x + cornerOff[i].x) * tileWidth) / tileWidth, y: (y - (tile.y + cornerOff[i].y) * tileHeight) / tileHeight}
